...
| 1.10 Modo de permissão do Claude Code (nunca Bypass; conector de escrita em aprovação manual) | Menor privilégio aplicado ao próprio agente: NIST SP 800-53, controle AC-6; documentação oficial da Anthropic sobre modos de permissão e `permissions.ask`/`deny` do Claude Code; OWASP Top 10 for LLM Applications (LLM01, injeção de prompt; LLM08, autonomia excessiva) |
